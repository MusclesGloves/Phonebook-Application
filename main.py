...
def insert():
    ...
    if(Name.isalpha()==False):
        messagebox.showerror('Error','Name should only include letters')
        flag=1
    if(Gender!='M' and Gender!='F' and Gender!='m' and Gender!='f'):
        messagebox.showerror('Error','Gender should only include either male or female')
        flag=1
    if(Telephone.isnumeric()==False):
        messagebox.showerror('Error','Telephone should only contain numbers')
        flag=1
    if('@gmail.com' not in Email):
        messagebox.showerror('Error','Enter the proper Email format')
        flag=1
    if(flag!=1):
        ...
        if Name == '' or Gender == '' or Telephone == '' or Email == '':
            messagebox.showwarning('data','Please fill the remaining fields')

        elif any(Telephone in s1 for s1 in view()):
            messagebox.showerror('data','Enter a unique value')
...
def to_update():
    try:
        ...
        def confirm():
            ...
            if(new_gender!='M' and new_gender!='F' and new_gender!='m' and new_gender!='f'):
                messagebox.showerror('Error','Gender should only include either male or female')
                flag=1
                
            if(new_telephone.isnumeric()==False):
                messagebox.showerror('Error','Telephone should only contain numbers')
                flag=1
            ...
            if(new_name=='' or new_email=='' or new_telephone=='' or new_gender==''):
                messagebox.showwarning('data','Please fill in all of the fields before modifying the values.')
                flag=1
...
def to_search():
    ...
    if(telephone == ''):
        messagebox.showerror('Error','Nothing is searched')
    elif any(telephone in s1 for s1 in view1()):
        ...
        messagebox.showinfo('Success','Data has been searched successfully')

    else:
        messagebox.showerror('Error','Enter an eligible Telephone value to be searched')
...
c_gender = ttk.Combobox(frame_down, width=25)
c_gender['values'] = ['', 'F', 'M']
